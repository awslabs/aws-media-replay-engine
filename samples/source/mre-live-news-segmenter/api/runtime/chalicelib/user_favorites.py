...
from chalice import Blueprint, ChaliceViewError
...
@user_favorites_api.route(
    "/samples/news/user-favorites/{program}/{event}/{user_name}",
    cors=True,
    methods=["POST", "PUT"],
...
def create_user_favorites(program, event, user_name):
    ...
    request format: /samples/news/user-favorites/{program}/{event}/{user_name}
...
def get_user_favorites(program, event, user_name):
    ...
    try:
        response = user_favorites_table.query(
            KeyConditionExpression=Key("program-event-user").eq(
                f"{program}#{event}#{user_name}"
            ),
...
@user_favorites_api.route(
    "/samples/news/user-favorites/{program}/{event}/{user_name}/{start}",
    cors=True,
    methods=["DELETE"],
    authorizer=authorizer,
)
def delete_user_favorites(program, event, user_name, start):
    ...
    request format: /samples/news/user-favorites/{program}/{event}/{user_name}/{start_time}
    headers: Content-Type=application/json
    ...
    """
    try:
        user_favorites_table.delete_item(
            Key={
                "program-event-user": f"{program}#{event}#{user_name}",
