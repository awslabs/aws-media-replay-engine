...
#    APPLICATION needs to be one of the following values: plugin-samples, model-samples, fan-experience-frontend, hls-harvester-sample, live-news-segmenter, live-news-segmenter-ui, live-news-segmenter-api
...
--app                           One of the following MRE Sample applications to deploy: plugin-samples, model-samples, fan-experience-frontend, hls-harvester-sample, live-news-segmenter, live-news-segmenter-ui, live-news-segmenter-api
...
if ! [[ "$app" =~ ^(plugin-samples|model-samples|fan-experience-frontend|hls-harvester-sample|live-news-segmenter|live-news-segmenter-ui|live-news-segmenter-api)$ ]]; then
  echo "ERROR: Invalid application name: $app. Should be one of the following: plugin-samples, model-samples, fan-experience-frontend, hls-harvester-sample, live-news-segmenter, live-news-segmenter-ui, live-news-segmenter-api"
...
if  [ "$app" = "live-news-segmenter" ]    || 
    [ "$app" = "live-news-segmenter-ui" ] || 
    [ "$app" = "live-news-segmenter-api" ]; then
  if [ "$app" = "live-news-segmenter" ] || [ "$app" = "live-news-segmenter-ui" ]; then
    # Get the email address to send login credentials for the UI
    echo "Please insert your email address to receive credentials required for the UI login:"
    read ADMIN_EMAIL
  fi
  ...
  build_dir="$PWD"
  source_dir="$build_dir/../source"
  live_news_segmenter_dir="$source_dir/live-news-segmenter"
  ...
  if [ "$app" = "live-news-segmenter" ] || [ "$app" = "live-news-segmenter-api" ]; then
    echo "------------------------------------------------------------------------------"
    echo "$api_stack_name"
    echo "------------------------------------------------------------------------------"
  ...
  if [ "$app" = "live-news-segmenter" ] || [ "$app" = "live-news-segmenter-ui" ]; then
    ...
    echo "Cloning the code from CodeCommit for Amplify deployment"
    cd "$source_ui_dir" || exit 1
    git clone "codecommit::$region://$(if [ ! -z $profile ]; then echo "$profile@"; fi)live-news-segmenter-frontend"
    rsync -r --exclude 'live-news-segmenter-frontend' --exclude 'node_modules' --exclude 'cdk' . live-news-segmenter-frontend
    
    echo "Pushing the code to CodeCommit for Amplify deployment"
    cd "$source_ui_dir"/live-news-segmenter-frontend
    ...
    git commit -m "Initial commit"
    git push --set-upstream origin master
    rm -rf "$source_ui_dir"/live-news-segmenter-frontend
  fi
  ...
  if [ "$app" = "live-news-segmenter" ] || [ "$app" = "live-news-segmenter-ui" ]; then
    ...
    echo "console under 'live-news-segmenter-frontend' application. Once deployed, click  "
    echo "on the URL within the application and login using the credentials sent to your "
    echo "email address.                                                                 "
  fi
  echo "-------------------------------------------------------------------------------"
