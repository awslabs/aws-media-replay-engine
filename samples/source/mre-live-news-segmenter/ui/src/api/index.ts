...
import { HttpResponseError, HttpResponseSuccess } from '@src/types';
import {
  getGeneralLoading,
  requestTracker,
  setGeneralLoading,
  ...
  try {
    ...
    switch (type) {
      case ApiMethods.POST:
        ...
        break;
      case ApiMethods.GET:
        response = await get({ apiName: api, path: url, options: options })
          .response;
        break;
      case ApiMethods.DEL:
        response = await del({ apiName: api, path: url, options: undefined })
          .response;
        break;
      case ApiMethods.PUT:
        response = await put({
