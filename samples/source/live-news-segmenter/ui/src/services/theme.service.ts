...
import { ApiNames, ApiMethods, ApiPaths, SortOrders } from '@src/enums';
...
export const themeServices: ThemeServices = {
    refreshEventThemes: (program, event, queryParams) => query(
        ApiMethods.GET,
        ApiNames.THEME,
    ...
    getChildThemes: (queryParams) => query(
        ApiMethods.GET,
        ApiNames.THEME,
        setApiPath(
            ApiPaths.GET_CHILD_THEMES,
            [
                queryParams.program,
                queryParams.event,
