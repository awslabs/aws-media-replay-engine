...
export const sortEvents = <T>(events: T[], sortOrder: SortOrders, converter: (e: string) => number): T[] => {
    if (events.length <= 1) {
        return events;
    }
    ...
    return (sortOrder === SortOrders.ASC
        ? [...sortEvents(leftArr, sortOrder, converter), pivot, ...sortEvents(rightArr, sortOrder, converter)]
        : [...sortEvents(rightArr, sortOrder, converter), pivot, ...sortEvents(leftArr, sortOrder, converter)]
    );
}