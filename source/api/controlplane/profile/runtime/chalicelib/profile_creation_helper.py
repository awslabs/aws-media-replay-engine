...
def get_plugin_configuration_ddb(plugin: dict) -> dict:
    ...
    try:
        ...
        get_key = {"Name": plugin["Name"], "Version": "v0"}
...
def enrich_plugin_from_plugin_definitions(parent_obj: dict, plugin_definitions: dict):
    ...
    parent_obj["Configuration"] = get_plugin_configuration(
        parent_obj, plugin_definitions
    )
    if "DependentPlugins" in parent_obj:
        for i, d_plugin in enumerate(parent_obj["DependentPlugins"]):
            parent_obj["DependentPlugins"][i]["Version"] = get_plugin_version(
                d_plugin, plugin_definitions
            )
            parent_obj["DependentPlugins"][i]["Configuration"] = (
                get_plugin_configuration(d_plugin, plugin_definitions)
            )
...
def enrich_profile(profile: dict, plugin_definitions: dict):
    ...
    if "Featurers" in profile:
        for p_index, featurer in enumerate(profile["Featurers"]):
            enrich_plugin_from_plugin_definitions(
                profile["Featurers"][p_index], plugin_definitions
            )
