...
class ReplayEngine:
    ...
    def __init__(self, event):
        ...
        self._replay_to_be_processed = (
            self.__event["ReplayRequest"] if "ReplayRequest" in self.__event else None
        )
        ...
        if self._replay_to_be_processed:
            if "ReplayId" in self._replay_to_be_processed:
                logger.append_keys(
                    replay_id=str(self._replay_to_be_processed["ReplayId"])
                )
        ...
        self.current_segment = (
            event["detail"]["Segment"] if self._is_catch_up_enabled() else {}
        )
        ...
        if self._is_catch_up_enabled():
            self.current_segment["HasFeedback"] = (
                True
                if event["detail"]["State"]
                in ["SEGMENT_CLIP_FEEDBACK", "OPTIMIZED_SEGMENT_CLIP_FEEDBACK"]
                else False
            )
        else:
        ...
        if self._replay_to_be_processed:
            if "ReplayId" in self._replay_to_be_processed:
                self.previous_replay_segments = self._dataplane.get_replay_segments(
                    self._event, self._program, self._replay_to_be_processed["ReplayId"]
                )

        self._clip_preview_feedback = self._dataplane.get_all_clip_preview_feedback(
            self._program, self._event, str(self._audio_track)
        )
        logger.info(
            f"all_clip_preview_feedback={json.dumps(self._clip_preview_feedback)}"
        )

    def __does_current_segment_have_any_replay_feature(
        self, current_segment_cache_file_name
    ):
        """
        ...
        for feature in self._replay_to_be_processed["Priorities"]["Clips"]:
            ...
            if "FeaturesDataPoints" in segment_mapping_as_json:
                ...
                if "0" in segment_mapping_as_json["FeaturesDataPoints"]:
                    ...
                    for feature_data_point in segment_mapping_as_json[
                        "FeaturesDataPoints"
                    ]["0"]:
                        if (
                            "Weight" in feature
                        ):  # For ClipBased settings in Replay, no weight attribute exists. This is Duration based.
                            # If any of the ReplayRequest Feature exists in the Segment
                            if (
                                feature["AttribName"] in feature_data_point
                                and feature["Weight"] > 0
                            ):
                                # Ex. SegmentBySceneAndSR | score_change | true
                                feature_condition = feature["Name"].split("|")[-1]
                                if (
                                    feature_data_point[feature["AttribName"]] is True
                                    if feature_condition.lower().strip() == "true"
                                    else False
                                ):
                                    return True
                        else:
                ...
                if (
                    str(self._audio_track)
                    in segment_mapping_as_json["FeaturesDataPoints"]
                ):
                    for feature_data_point in segment_mapping_as_json[
                        "FeaturesDataPoints"
                    ][str(self._audio_track)]:
    ...
    def get_unique_segment_features_for_catchup(
        self, segments_with_features_from_new_cache_files
    ):
        """
        ...
        try:
            ...
            s3_client.download_file(
                CACHE_BUCKET,
                f"replay-result-seg-cache/{self._replay_to_be_processed['ReplayId']}/SegmentsFeaturesCache.json",
                local_file,
            )
            ...
            segments_with_features.extend(segments_with_features_from_replay_cache_file)

            for (
                seg
            ) in (
                segments_with_features_from_new_cache_files
            ):  # this should be the last 10 segments
        ...
        # else:
            ...
            logger.info(
                "SEGMENT_FEATURE_MAPPING_CACHE_DOWNLOAD_ERROR - SegmentsFeaturesCache.json not downloaded , may not exist, which is ok if none of the previous segments having features."
            )
            segments_with_features.extend(segments_with_features_from_new_cache_files)
    ...
    def should_segment_be_force_included(self, segment_start_time, segment):
        ...
        does_replay_force_segments_inclusion = (
            False
            if "IncludeLikedSegments" not in self._replay_to_be_processed
            else self._replay_to_be_processed["IncludeLikedSegments"]
        )
        if does_replay_force_segments_inclusion:
            ...
            for clip_feedback in self._clip_preview_feedback:
                if "Start" in clip_feedback:
                    if float(str(segment_start_time)) == float(
                        str(clip_feedback["Start"])
                    ):
                        ...
                        if "OptoStart" in segment and "OptoEnd" in segment:
                            if "OptimizedFeedback" in clip_feedback:
                                if "Feedback" in clip_feedback["OptimizedFeedback"]:
                                    return (
                                        True
                                        if clip_feedback["OptimizedFeedback"][
                                            "Feedback"
                                        ].lower()
                                        == "like"
                                        else False
                            ...
                            if "OriginalFeedback" in clip_feedback:
                                if "Feedback" in clip_feedback["OriginalFeedback"]:
                                    return (
                                        True
                                        if clip_feedback["OriginalFeedback"][
                                            "Feedback"
                                        ].lower()
                                        == "like"
                                        else False
    ...
    def is_segment_disliked(self, segment_start_time, segment):
        does_replay_force_segments_exclusion = (
            False
            if "IgnoreDislikedSegments" not in self._replay_to_be_processed
            else self._replay_to_be_processed["IgnoreDislikedSegments"]
        )
                ...
                if float(str(segment_start_time)) == float(str(clip_feedback["Start"])):
                    ...
                    if "OptoStart" in segment and "OptoEnd" in segment:
                        if "OptimizedFeedback" in clip_feedback:
                            if "Feedback" in clip_feedback["OptimizedFeedback"]:
                                if (
                                    clip_feedback["OptimizedFeedback"][
                                        "Feedback"
                                    ].lower()
                                    == "dislike"
                                ):
                        ...
                        if "OriginalFeedback" in clip_feedback:
                            if "Feedback" in clip_feedback["OriginalFeedback"]:
                                if (
                                    clip_feedback["OriginalFeedback"][
                                        "Feedback"
                                    ].lower()
                                    == "dislike"
                                ):
    ...
    def should_segment_be_considered_after_initial_inclusion_exclusion(
        self, segment_start_time, segment
    ):
        """
    ...
    def initialize_segment_features(self):
        ...
        if self._is_catch_up_enabled():
            ...
            if not self.is_segment_disliked(
                self.current_segment["Start"], self.current_segment
            ):
                ...
                if not self.should_segment_be_considered_after_initial_inclusion_exclusion(
                    self.current_segment["Start"], self.current_segment
                ):

                    # if Current Segment is not to be force included, check if it has any Features we are looking for
                    if not self.should_segment_be_force_included(
                        self.current_segment["Start"], self.current_segment
                    ):

                        logger.info(
                            f"CURRENT Segment with StartTime {str(self.current_segment['Start'])} is NOT set for FORCE INCLUSION. Cache file name {current_segment_cache_file_name}. Proceeding to check if it has any Replay features..."
                        )
                        ...
                        if not self.__does_current_segment_have_any_replay_feature(
                            current_segment_cache_file_name
                        ):
                            ...
                            logger.info(
                                f"CURRENT Segment with StartTime {str(self.current_segment['Start'])} has no ReplayRequest Feature's. IGNORING further processing for this Segment. Cache file name {current_segment_cache_file_name}."
                            )

                            return False
                        else:
                            logger.info(
                                f"CURRENT Segment with StartTime {str(self.current_segment['Start'])} cache file name {current_segment_cache_file_name} HAS ONE OR MORE FEATURES !!! "
                            )

                    else:
                        logger.info(
                            f"CURRENT Segment with StartTime {str(self.current_segment['Start'])} is set for FORCE INCLUSION. Cache file name {current_segment_cache_file_name}. Will be considered to be added to replay !!! "
                        )
                else:
                    logger.info(
                        f"CURRENT Segment with StartTime {str(self.current_segment['Start'])} cache file name {current_segment_cache_file_name} is to be removed/added since it was INCLUDED/EXCLUDED MANUALLY. Proceeding further ..."
                    )
            else:
                logger.info(
                    f"CURRENT Segment with StartTime {str(self.current_segment['Start'])} is set for MANUAL REMOVAL. cache file name {current_segment_cache_file_name}. Proceeding further ..."
                )

        # Step 5 - Get all segments with Features mapped from Cache. This is a Multi-threaded process

        logger.info(
            "Starting the Multi threaded process to find ReplayRequest Features in Segments from Cache ..."
        )
        replay_feature_processor = ReplayFeatureProcessor(
            ...
            (
                self._replay_to_be_processed["SegmentsToBeIgnored"]
                if "SegmentsToBeIgnored" in self._replay_to_be_processed
                else []
            ),
            self._audio_track,
            self._event,
        ...
        segments_with_features_from_new_cache_files = (
            replay_feature_processor.find_features_in_cached_files()
        )
        logger.info(
            f"segments_with_features_from_new_cache_files={json.dumps(segments_with_features_from_new_cache_files)}"
        )
        ...
        logger.info(
            f"SORTED _all_segments_with_features before re-calculating Scores = {json.dumps(self._all_segments_with_features)}"
        )
        return True
    ...
    def cache_replay_calc_segment_feature_mapping_in_s3(self):
        ...
        s3_client.upload_file(
            f"/tmp/{self._replay_to_be_processed['ReplayId']}-SegmentsFeaturesCache.json",
            CACHE_BUCKET,
            new_replay_segment_feature_cache_location,
        )
    ...
    def __get_s3_key_prefixes(self):
        ...
        cache_discovery = CacheDiscovery(
            CACHE_BUCKET, self._program, self._event, self._profile
        )
    ...
    def are_segments_different(self, new_replay_segments):
        ...
        logger.info(
            f"Comparing New and Prev Segments - Prev Segments = {json.dumps(self.previous_replay_segments)}, New Segments = {json.dumps(new_replay_segments)}"
        )
            ...
            if float(str(segment_start_time)) == float(str(clip_feedback["Start"])):
                ...
                if "OptoStart" in segment and "OptoEnd" in segment:
                    if "OptimizedFeedback" in clip_feedback:
                        if "Feedback" in clip_feedback["OptimizedFeedback"]:
                            if (
                                clip_feedback["OptimizedFeedback"]["Feedback"].lower()
                                == "dislike"
                            ):
                                return True
                else:
                    if "OriginalFeedback" in clip_feedback:
                        if "Feedback" in clip_feedback["OriginalFeedback"]:
                            if (
                                clip_feedback["OriginalFeedback"]["Feedback"].lower()
                                == "dislike"
                            ):
                                return True
        return False
    ...
    def _create_replay(self):
        ...
        if not self._is_replay_duration_based():
            ...
            logger.info(
                f"CLIP BASED - FINAL SEGMENTS SAVED IN REPLAY RESULT - for {self._replay_to_be_processed['ReplayId']}--{json.dumps(self._all_segments_with_features)}"
            )
            ...
            if not replay_res:
                logger.info(
                    "Clip based replay. Replay results have not been persisted. Check for Race condition!!"
                )
                return False
            ...
            replay_request_duration = self.__event["ReplayRequest"][
            ...
            if (
                ...
                in self.__event["ReplayRequest"]["DurationbasedSummarization"]
            ):
                replay_request_duration = (
                    replay_request_duration
                    + self.__event["ReplayRequest"]["DurationbasedSummarization"][
                        "ToleranceMaxLimitInSecs"
                    ]
                )
                logger.info(
                    f"Total Replay duration set to {replay_request_duration} secs based on Tolerance value."
                )
            ...
            duration_result, duration = (
                self._does_total_duration_of_all_segments_exceed_configured(
                    replay_request_duration
                )
            )
            ...
            if not duration_result:
                logger.info(
                    f"Potential Segments which can be included in Replay = {json.dumps(self._all_segments_with_features)}"
                )
                if len(self._all_segments_with_features) > 0:
                    ...
                    if not self.are_segments_different(sorted_final_segments):
                        logger.info(
                            f"CHECK 1 - PREV AND NEW SEGMENTS ARE EQUAL .. not persisting the new segments nor gen clips --{json.dumps(sorted_final_segments)}"
                        )
                        return False
                    logger.info(
                    ...
                    logger.info(
                        f"CHECK 1 - FINAL SEGMENTS SAVED IN REPLAY RESULT -  for {self._replay_to_be_processed['ReplayId']}--{json.dumps(self._all_segments_with_features)}"
                    )
                    replay_res = self._persist_replay_metadata(
                        self._all_segments_with_features, debuginfo
                    )
                    logger.info(
                        f"CHECK 1 - Total duration {duration} secs of chosen Segments is less than Replay Request duration {replay_request_duration} secs."
                    )
                else:
                    ...
                    logger.info(
                        "CHECK 1 - We found Segments having at least one Feature, but they have Zero Weights. Ignoring replay creation."
                    )
                    return False
                ...
                if not self.__event["ReplayRequest"]["DurationbasedSummarization"][
                    "EqualDistribution"
                ]:
                    ...
                    logger.info(
                        f"---- AFTER DESC SORT Before Filtering based on Duration and Score - AllSegmentsWithFeatures ----{json.dumps(sorted_segments)}"
                    )
                    ...
                    for segment in sorted_segments:
                        ...
                        if (
                            ...
                            total_duration += segment_duration
                            logger.info(
                                f"SEGMENT ADDED - Segment Choosen = {segment}, total_duration = {total_duration}"
                            )
                        else:
                            logger.info(
                                f"SEGMENT NOT ADDED - Segment not Choosen = {segment}, total_duration if chosen = {total_duration + segment_duration}"
                            )

                    logger.info(
                        f"Duration of all selected segments is {total_duration} secs"
                    )
                    ...
                    if not self.are_segments_different(sorted_final_segments):
                        logger.info(
                            f"CHECK 2 - PREV AND NEW SEGMENTS ARE EQUAL .. not persisting the new segments nor gen clips --{json.dumps(sorted_final_segments)}"
                        )
                        return False

                    logger.info(
                        f"CHECK 2 - FINAL SEGMENTS SAVED IN REPLAY RESULT - for {self._replay_to_be_processed['ReplayId']}--{json.dumps(sorted_final_segments)}"
                    )
                    replay_res = self._persist_replay_metadata(
                    ...
                    end_of_last_segment_secs = (
                        last_segment["OptoEnd"]
                        if self._is_segment_optimized(last_segment)
                        else last_segment["End"]
                    )
                    ...
                    total_segment_duration_in_secs = (
                        end_of_last_segment_secs - start_of_first_segment_secs
                    )
                    ...
                    if total_segment_duration_in_secs > 3600:
                        no_of_time_groups = (
                            math.ceil(total_segment_duration_in_hrs) * 2
                        ) + TIMEGROUP_MULTIPLIER
                    else:
                            ...
                            time_groups.append(
                                {
                                    ...
                                    "TimeGroupEndInSecs": start_of_first_segment_secs
                                    + time_frame_in_time_group,
                                }
                            )
                            total += (
                                start_of_first_segment_secs + time_frame_in_time_group
                            )
                        else:
                            time_groups.append(
                                {
                                    "TimeGroupStartInSecs": total + 0.001,
                                    "TimeGroupEndInSecs": total
                                    + time_frame_in_time_group,
                                }
                            )
                    ...
                    while True:
                        segments_per_pass = []
                        logger.info(
                            f"Processing Index {segment_index} from each Time group segment list"
                        )
                        for timegroup in time_groups:

                            if (
                                str(timegroup["TimeGroupStartInSecs"])
                                in time_groups_with_no_segments_available_or_duration_met
                            ):
                                continue

                            segments_within_timegroup = (
                                self._get_segments_with_features_within_timegroup(
                                    timegroup
                                )
                            )
                            logger.info(
                                f"segments_within_timegroup={segments_within_timegroup}, TimeGroupStartInSecs = {str(timegroup['TimeGroupStartInSecs'])}"
                            )
                            ...
                            if len(sorted_segments) > 0:
                                try:
                                    ...
                                    segments_per_pass.append(
                                        {
                                            "TimeGroupStartInSecs": str(
                                                timegroup["TimeGroupStartInSecs"]
                                            ),
                                            "Segment": segment,
                                        }
                                    )
                                    ...
                                    logger.info(
                                        f"Exception - Segment with specific Index not found in a Timegroup.  Index = {segment_index}, TimeGroupStartInSecs = {timegroup['TimeGroupStartInSecs']}. This is expected as each Timegroup has variable number of segments."
                                    )
                                    time_groups_with_no_segments_available_or_duration_met.append(
                                        str(timegroup["TimeGroupStartInSecs"])
                                    )
                            else:
                                logger.info(
                                    f"No segments available in this TimeGroup. This is not an Error. Index = {segment_index}, TimeGroupStartInSecs = {timegroup['TimeGroupStartInSecs']}"
                                )
                                time_groups_with_no_segments_available_or_duration_met.append(
                                    str(timegroup["TimeGroupStartInSecs"])
                                )

                        sorted_segments_per_pass = sorted(
                            segments_per_pass,
                            key=lambda x: x["Segment"]["Score"],
                            reverse=True,
                        )
                        for seg in sorted_segments_per_pass:
                            ...
                            if (
                                ...
                                total_duration += segment_duration
                                logger.info(
                                    f"SEGMENT ADDED - Processing Index = {segment_index}, TimeGroupStartInSecs = {seg['TimeGroupStartInSecs']}, Segment Choosen = {seg['Segment']}, total_duration = {total_duration}"
                                )
                            else:
                                logger.info(
                                    f"SEGMENT NOT ADDED - Processing Index = {segment_index}, TimeGroupStartInSecs = {seg['TimeGroupStartInSecs']}, Segment not Choosen = {seg['Segment']}, total_duration if chosen = {total_duration + segment_duration}"
                                )
                        ...
                        if not final_segments:
                            logger.info(
                                "HOW DID THIS OCCUR? - We should have got at east one segment in one of the time groups"
                            )
                            break
                    ...
                    logger.info(
                        f"Duration of all selected segments is {total_duration} secs. Segments selected so far = {json.dumps(final_segments)}"
                    )
                    ...
                    # Now that we have the segments that are within the replay, lets compare to see if
                    # these segments are any different than the segments which have already been persisted for the replay request
                    if not self.are_segments_different(
                        final_segments_across_timegroups
                    ):
                        logger.info(
                            f"CHECK 3 - PREV AND NEW SEGMENTS ARE EQUAL .. not persisting the new segments nor gen clips --{json.dumps(final_segments_across_timegroups)}"
                        )
                        return False

                    logger.info(
                        f"CHECK 3 - FINAL SEGMENTS SAVED IN REPLAY RESULT - for {self._replay_to_be_processed['ReplayId']}--{json.dumps(final_segments_across_timegroups)}"
                    )
    ...
    def _get_segments_with_features_within_timegroup(self, timegroup):
        ...
        for segment in self._all_segments_with_features:
            ...
            logger.info(
                f"Checking if segment start {segment_startTime} Falls in TimeGroupStartInSecs = {timegroup['TimeGroupStartInSecs']} and {timegroup['TimeGroupEndInSecs']}"
            )
            if (
                segment_startTime >= timegroup["TimeGroupStartInSecs"]
                and segment_startTime <= timegroup["TimeGroupEndInSecs"]
            ):
                segments_within_timegroup.append(segment)
                logger.info(
                    f"Added segment start {segment_startTime} for TimeGroupStartInSecs = {timegroup['TimeGroupStartInSecs']}"
                )
    ...
    def _does_total_duration_of_all_segments_exceed_configured(
        self, replay_request_duration
    ):
        duration = 0
        ...
        return (
            (True, duration)
            if duration > replay_request_duration
            else (False, duration)
        )
    ...
    def _persist_replay_metadata(self, segmentInfo, additionalInfo=None):
        ...
        for segment_feature in segmentInfo:
            rep_res = {
                ...
                "Score": (
                    segment_feature["Score"] if "Score" in segment_feature else "-"
                ),
        ...
        replay_result_payload["AdditionalInfo"] = (
            additionalInfo if additionalInfo is not None else []
        )
        ...
        replay_result_payload["LastSegmentStartTime"] = (
            self.current_segment["Start"] if self._is_catch_up_enabled() else 0
        )
        replay_result_payload["HasFeedback"] = self.current_segment["HasFeedback"]

        if self.current_segment["HasFeedback"]:
            logger.info(
                f"SEGMENT that initiated the replay with StartTime - {self.current_segment['Start']} HAS FEEDBACK. PERSISTED TO DDB"
            )
        else:
            logger.info(
                "REPLAY PERSISTED TO DDB. This replay was not initiated due to a Manual clip Feedback override."
            )
    ...
    def __mark_replay_in_progress(self):
        self._controlplane.update_replay_request_status(
            self._program,
            self._event,
            self.__event["ReplayRequest"]["ReplayId"],
            "In Progress",
        )
    ...
    def _get_event(self):
        return self._controlplane.get_event(
            self._event,
            self._program,
        )
    ...
    def _get_featurers_by_profile(self):
        ...
        self._featurers = [
            feature_dict["Name"] for feature_dict in self._profile["Featurers"]
        ]
    ...
    def _get_all_replays_for_opto_segment_end(self):
        ...
        """
        return self._controlplane.get_all_replay_requests_for_event_opto_segment_end(
            self._program, self._event, int(self._audio_track)
        )
    ...
    def _is_replay_duration_based(self):
        ...
        return (
            False
            if self._replay_to_be_processed["ClipfeaturebasedSummarization"]
            or "SpecifiedTimestamps" in self._replay_to_be_processed
            else True
        )
