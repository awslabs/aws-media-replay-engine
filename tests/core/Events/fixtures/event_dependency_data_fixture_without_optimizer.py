...
# This fixture lets us run tests in parallel while enabling Plugins and Profile without a Optimizer to be registered only once per test session
...
def create_event_dependent_data_with_no_opto(tmp_path_factory, worker_id, name="create_event_dependent_data_with_no_opto"):
                
    if not worker_id:
        # not executing in with multiple workers, just return the channel id list and let
        # pytest's fixture caching do its job
        return create_event_dependent_data_with_no_opto()
    
    # get the temp directory shared by all workers
    root_tmp_dir = tmp_path_factory.getbasetemp().parent

    fn = root_tmp_dir / "event-data-no-opto.json"
    with FileLock(str(fn) + ".lock"):
        if fn.is_file():
            ...
            create_event_dependency_data_with_no_opto()
    return data
...
def register_profile():
    ...
    #Remove the optimizer from this profile
    config.pop("Optimizer")
    config["Name"] = "TestSuite-EventTestPassThroughProfile-WithoutOptimizer"

    call_api(path="profile", api_method="POST", api_body=json.dumps(config))
    return config


def create_event_dependency_data_with_no_opto():
    ...
    try:
        call_api(path="profile/TestSuite-EventTestPassThroughProfile-WithoutOptimizer", api_method="GET")
    except NotFoundError as e:
        register_profile()
